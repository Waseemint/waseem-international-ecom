...
from customkit.models import CustomFonts, CustomColor, Designs, Pattern
...
def custom_product_detail(request, product_slug):
    ...
    fonts = CustomFonts.objects.all()
    colors=CustomColor.objects.all()
    designs=Designs.objects.all().order_by('-id')
    patteren=Pattern.objects.all().order_by('-id')
