...
class Product(models.Model):
    ...
    category = models.ForeignKey(ChildCategory, on_delete=models.CASCADE,related_name='store_products')
    created_date = models.DateTimeField(auto_now_add=True)
    modified_date = models.DateTimeField(auto_now=True)
    has_variations = models.BooleanField(default=True)  # New field to determine if the product has variations
...
class ReviewRating(models.Model):
    product = models.ForeignKey(Product, on_delete=models.CASCADE)
    user = models.ForeignKey(Account, on_delete=models.CASCADE,related_name='store_review_ratings')
...
class Price(models.Model):
    customization=models.CharField(max_length=10, null=True, blank=True)
    trouser=models.CharField(max_length=10, null=True, blank=True)
    ...
    def __str__(self):
        return str(f"Custom Prices {self.id}")
